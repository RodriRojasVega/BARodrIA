// src/modules/cocteles/components/CoctelList.tsx
...
export function CoctelList({ data, catalogos, isLoading, onNuevo, onVerDetalle }: CoctelListProps) {
  ...
  // Filtrado y ordenamiento reactivo (Tipado Estricto)
  const datosProcesados = useMemo(() => {
    ...
    // Función selectora pura para evitar reasignaciones (let) y código muerto
    const getValorOrdenamiento = (coctel: Coctel): string | number => {
      if (columnaOrden === 'categoria_nombre') return catalogos.categorias.find(c => c.id === coctel.categoria_id)?.nombre || '';
      if (columnaOrden === 'familia_nombre') return catalogos.familias.find(c => c.id === coctel.familia_id)?.nombre || '';
      if (columnaOrden === 'tecnica_nombre') return catalogos.tecnicas.find(c => c.id === coctel.tecnica_id)?.nombre || '';
      if (columnaOrden === 'soporte_nombre') return catalogos.soportes.find(c => c.id === coctel.soporte_id)?.nombre || '';
      
      const raw = coctel[columnaOrden as keyof Coctel];
      return typeof raw === 'string' || typeof raw === 'number' ? raw : 0;
    };

    // 2. Ordenar
    filtrados.sort((a, b) => {
      const valA = getValorOrdenamiento(a);
      const valB = getValorOrdenamiento(b);
