// src/modules/cocteles/components/CoctelForm.tsx
...
export function CoctelForm({ coctelAEditar, onCerrar, onGuardar }: CoctelFormProps) {
  ...
  // Cumpliendo Regla 6: Inicialización perezosa estricta
  const [formData, setFormData] = useState<Partial<Coctel>>(() => {
    if (coctelAEditar) return { ...coctelAEditar };
    return {
      nombre: '', 
      slug: '', 
      categoria_id: 0, 
  ...
  // Carga de dependencias relacionales
  useEffect(() => {
    let isMounted = true;
  ...
  // Cumpliendo Regla 1: Cero "any", uso estricto de genéricos
  const handleInputChange = <K extends keyof Coctel>(field: K, value: Coctel[K]) => {
    setFormData(prev => ({
      ...prev,
      [field]: value
    }));
  ...
  // Cumpliendo Regla 1: Tipado estricto en handlers
  const updateIngrediente = (index: number, field: keyof IngredienteRow, value: string | number) => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div>
              ...
              <Select value={formData.categoria_id || ''} onChange={e => handleInputChange('categoria_id', Number(e.target.value))}>
                ...
                {catalogos.categorias.map(c => <option key={c.id} value={String(c.id)}>{c.nombre}</option>)}
              </Select>
            </div>
            <div>
              ...
              <Select value={formData.familia_id || ''} onChange={e => handleInputChange('familia_id', Number(e.target.value))}>
                ...
                {catalogos.familias.map(f => <option key={f.id} value={String(f.id)}>{f.nombre}</option>)}
              </Select>
            </div>
            <div>
              ...
              <Select value={formData.soporte_id || ''} onChange={e => handleInputChange('soporte_id', Number(e.target.value))}>
                ...
                {catalogos.soportes.map(s => <option key={s.id} value={String(s.id)}>{s.nombre}</option>)}
              </Select>
            </div>
            <div>
              ...
              <Select value={formData.hielo_id || ''} onChange={e => handleInputChange('hielo_id', Number(e.target.value))}>
                ...
                {catalogos.hielos.map(h => <option key={h.id} value={String(h.id)}>{h.nombre}</option>)}
              </Select>
            </div>
            <div>
              ...
              <Select value={formData.tecnica_id || ''} onChange={e => handleInputChange('tecnica_id', Number(e.target.value))}>
                ...
                {catalogos.tecnicas.map(t => <option key={t.id} value={String(t.id)}>{t.nombre}</option>)}
              </Select>
            </div>
          </div>
        </section>
                  ...
                  <div className="col-span-12 sm:col-span-8">
                    <Select value={ing.insumo_id || ''} onChange={e => updateIngrediente(idx, 'insumo_id', e.target.value)}>
                      ...
                      {insumosGlobales.map(i => (
                        <option key={i.id} value={String(i.id)}>{i.nombre} (${i.costo_unitario}/{i.unidad_medida})</option>
                      ))}
                    </Select>
                  </div>
        ...
        <section className="space-y-4 pt-2">
          ...
          <div className="space-y-4">
            <div>
              ...
              <Textarea 
                value={formData.notas_inspiracion || ''} 
                onChange={e => handleInputChange('notas_inspiracion', e.target.value)} 
                rows={3}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                ...
                <Textarea value={formData.notas_vista || ''} onChange={e => handleInputChange('notas_vista', e.target.value)} rows={2} placeholder="Ej: Brillante, tono ámbar..." />
              </div>
              <div>
                <label className="block text-xs font-mono uppercase text-warning mb-1">👃 Nariz (Aromas)</label>
                <Textarea value={formData.notas_nariz || ''} onChange={e => handleInputChange('notas_nariz', e.target.value)} rows={2} placeholder="Ej: Notas cítricas y vainilla..." />
              </div>
              <div>
                <label className="block text-xs font-mono uppercase text-info mb-1">👄 Boca (Paladar)</label>
                <Textarea value={formData.notas_boca || ''} onChange={e => handleInputChange('notas_boca', e.target.value)} rows={2} placeholder="Ej: Sedoso, final seco..." />
              </div>
            </div>
