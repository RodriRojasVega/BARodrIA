// src/modules/cocteles/components/CoctelDetail.tsx
...
export function CoctelDetail({ coctel, onVolver, onEditar, onEliminar }: CoctelDetailProps) {
  ...
  // Regla de Oro: Fetching seguro con control de montaje para evitar memory leaks
  useEffect(() => {
    let isMounted = true;
    ...
    async function cargarDatosRelacionales() {
      ...
      try {
        const [resCat, resFam, resSop, resTec] = await Promise.all([
          supabase.from('categorias').select('nombre').eq('id', Number(coctel.categoria_id)).single(),
          supabase.from('familias').select('nombre').eq('id', Number(coctel.familia_id)).single(),
          supabase.from('soportes').select('nombre').eq('id', Number(coctel.soporte_id)).single(),
          supabase.from('tecnicas').select('nombre').eq('id', Number(coctel.tecnica_id)).single()
        ]);
  ...
  }, [coctel.id, coctel.categoria_id, coctel.familia_id, coctel.soporte_id, coctel.tecnica_id]);
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-sm text-foreground italic bg-surface p-4 rounded-xl border border-border leading-relaxed shadow-sm">
                    {coctel.notas_inspiracion || 'Sin receta histórica o storytelling registrado.'}
                  </p>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <InfoCard title="👁️ Vista (Aspecto)" value={coctel.notas_vista || '-'} variant="success" />
                  <InfoCard title="👃 Nariz (Aromas)" value={coctel.notas_nariz || '-'} variant="warning" />
                  <InfoCard title="👄 Boca (Paladar)" value={coctel.notas_boca || '-'} variant="info" />
                </div>
